...
public class Gaussian {
    ...
    private static Apfloat phi(Apfloat x) {
    	Apfloat exponent = ApcomplexMath.pow(x, new Apint(2)).negate().divide(new Apint(2)).real();
    	return ApfloatMath.exp(exponent). 
    			divide(ApfloatMath.sqrt(new Apint(2).multiply(ApfloatMath.pi(16))));
	}
	...
	public static Apfloat Phi(Apfloat z) {
		
		if (z.compareTo(new Apfloat(-30.0)) == -1) return new Apfloat(0.0);
        if (z.compareTo(new Apfloat(30.0)) == 1) return new Apfloat(1.0);
        
        Apfloat sum = new Apfloat(0.0), term = z;
        
        for (int i = 3; sum.add(term).compareTo(sum) != 0; i += 2) {
            sum  = sum.add(term);
            term = term.multiply(z).multiply(z).divide(new Apfloat(i));
        }
        
        return new Apfloat(0.5).add(sum.multiply(phi(z)));
    ...
    public static double PhiInverse(double y) {
        return PhiInverse(y, Math.pow(10, -12), -30, 30);
    } 
